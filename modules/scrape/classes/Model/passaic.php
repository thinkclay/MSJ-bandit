 ...
 * @package Scrape
 * @author Bryan Galli
 * @url http://www.pcsheriff.org/index.php?option=com_wrapper&view=wrapper&Itemid=118
 * http://65.51.122.207/ArchonixXJailPublic/Client/ReportViewer.aspx?ReportType=BOOKINGLOG,10/31/2011,11/07/2011,0%20,_blank
 */
class Model_Passaic extends Model_Scrape
{
    private $scrape     = 'passaic';
    private $state      = 'new jersey';
    private $cookies    = '/tmp/passaic_cookies.txt';
    ...
    */
    ...
    {
    	$county = 'passaic';
    	$home_url = 'http://65.51.122.207/ArchonixXJailPublic/Default.aspx';
		//http://65.51.122.207/ArchonixXJailPublic/Default.aspx
        $next_url = 'http://65.51.122.207/ArchonixXJailPublic/Client/ReportViewer.aspx?ReportType=BOOKINGLOG,10/31/2011,11/07/2011,0%20,_blank';
		$more = 'http://65.51.122.207/ArchonixXJailPublic/Client/ReportViewer.aspx?ReportType=BOOKINGLOG,10/31/2011,11/07/2011,0%20,_blank';
        $home = $this->curl_to_url($home_url);
	
		$next = $this->curl_to_url($next_url);
		echo $home;
		
		preg_match('/ADAMS/Uis', $next, $matches);
		print_r($matches);
		exit;
        $check = preg_match_all('/a\shref\=\"(.*)\"/Uis', $home, $matches);
		if($check)
		{
    ...
    function curl_to_url($url)
    {
		...
		curl_setopt($ch, CURLOPT_REFERER, 'http://65.51.122.207/ArchonixXJailPublic/Client/BookingLogReport.aspx');
		curl_setopt($ch, CURLOPT_COOKIE, 'ASP.NET_SessionId=4cq51zvy0vkbpl0i5inuddag');
    ...
    */
    ...
    {
    	$theurl = "http://www.co.yamhill.or.us/sheriff/inmates/" . $offender_link;
    	$details_page = $this->curl_to_url($theurl);
    	$check = preg_match('/Booking.*\<b\>(.*)\&/Uis', $details_page, $booking_id);
		...
		if(isset($booking_id))
		{
			...
			if(!$offender->loaded())
			{
				$check = preg_match('/\<\/b\>Inmate\sName.*\>(.*)\</Uis', $details_page, $name);
				if($check)
				{
					...
					$check = preg_match_all('/\<\/b\>Offense.*\>(.*)\</Uis', $details_page, $charges);
					if($check)
					{
						$charges = $this->clean_string_utf8($charges[1]);
						$check = preg_match('/Booking\sDate\s\:\s\<b\>(.*)\</Uis', $details_page, $booking_date);
						if($check)
						{
							...
							if($check)
							{
								...
								$image_link = 'http://www.co.yamhill.or.us/sheriff/inmates/' . $image_id;
								...
								$extra_fields = array();
								$check = preg_match('/Age.*\<b\>.*Sex.*<b\>(.*)\</Uis', $details_page, $match);
								if($check)
								{
									$extra_fields['gender'] = $this->gender_mapper($this->clean_string_utf8($match[1]));
								}
								
								//system abbreviator boilerplate begin
								if(!empty($charges))
								{
								...
								// system abbreviator boilerplate end
