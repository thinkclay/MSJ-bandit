 ...
 */
class Model_Summit_Test extends Model_Scrape
{
    private $scrape     = 'summit'; //name of scrape goes here
	private $county 	= 'summit'; // if it is a single county, put it here, otherwise remove this property
    private $state      = 'ohio'; // state goes here
    private $cookies    = '/tmp/summit_cookies.txt'; // replace with <scrape name>_cookies.txt
    ...
    */
    function scrape() 
    {
    	$tmp_folder = '/raw/ohio/summit/';
		$handle = fopen($tmp_folder . 'summit.html', 'r');
		$text = fread($handle, filesize($tmp_folder . 'summit.html'));
		echo $text;
		exit;
		$pdf_file = $tmp_folder . 'summit.pdf';
		$text_file = $tmp_folder . 'summit.txt';
		$index = $this->curl_index();
		//echo $index;
		$check = preg_match('/a\shref\=\"(http\:\/\/www\.co\.summit\.oh\.us\/sheriff\/Jail\_Records\/Matrix.*)\"/Uis', $index, $match);
		if ( ! $check)
			return false;
		$pdf_url = $match[1];
		...
		shell_exec('pdftotext -layout ' . $pdf_file . ' ' . $text_file);
		shell_exec('pdfimages -j ' . $pdf_file . ' ' . $tmp_folder);
		$handle = fopen($text_file, 'r');
		$text = fread($handle, filesize($text_file));
		fclose($handle);
		echo $text; 
		exit;
		$lines = preg_split("/((\r?\n)|(\r\n?))/", $text);
    ...
    */
    function curl_index()
    {
        $url = 'http://scsojms.summitoh.net/matrixjms/publicsearch/masterfilenamesearch.aspx';  // this will be the url to the index page
        $ch = curl_init();   
