 ...
 * @package Scrape
 * @author 	Winter King	
 * @url 	http://www.brevardsheriff.com/dar/inmatephotos/index.html
 */
class Model_Brevard extends Model_Scrape
{
    private $scrape     = 'brevard'; // Name of scrape goes here
	private $county 	= 'brevard'; // if it is a single county, put it here, otherwise remove this property
    private $state      = 'florida'; // state goes here
    private $cookies    = '/tmp/brevard_cookies.txt'; // replace with <scrape name>_cookies.txt
    ...
    */
    function scrape() 
    {
		...
		$check = preg_match_all('/href\=\"(.*)\"/Uis', $index, $matches); // get an array of booking_ids
		if ($check)
		{
			$booking_ids = $matches[1];
			foreach ($booking_ids as $key => $booking_id)
			{
				$booking_ids[$key] = preg_replace('/[^0-9]/', '', $booking_id);
			}
			array_shift($booking_ids);
    ...
    */
    function curl_index()
    {
        $url = 'http://www.brevardsheriff.com/dar/inmatephotos/index.html';  // this will be the url to the index page
        $ch = curl_init();   
    ...
    * curl_details
    * 
    * @url $url = 'http://www.brevardsheriff.com/dar/inmatephotos/$bid';
    *   
    */
    function curl_details($booking_id)
    {
        $url = 'http://www.brevardsheriff.com/dar/inmatephotos/'.$booking_id . '.html';
        $ch = curl_init();   
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_COOKIEFILE, $this->cookies);
        curl_setopt($ch, CURLOPT_COOKIEJAR, $this->cookies);
    ...
    */
    function extraction($details, $booking_id)
    {
		...
		$check = preg_match('/<table.*<table.*<table.*<table.*(<table.*<\/table>)/Uis', $details, $match);
		if ( ! $check)
		{
			return 101;
		}
		
		$check = preg_match('/<p.*<center><font.*<I>(.*)<br><center>/Uis', $details, $match);
		if ( ! $check)
		{
			return 101;
		}
		// Just get first and last name here
		...
		// get booking date
		$check = preg_match('/ARREST\sDATE\:\s(.*)<br>/Uis', $details, $match);
		if ( ! $check)
		{ 
			return 101; 	
		}
		...
		$check = preg_match('/CURRENT\sCHARGES.*<b>(.*)</Uis', $details, $match);
		if ( ! $check)
		{
			return 101;	
		}
		...
		*/
		...
		$check = preg_match('/(Images\/.*)\"/Uis', $details, $match);
		if ( ! $check)
		{
			return 102; // Image link not found
		}
	    $image_link = 'http://www.brevardsheriff.com/dar/inmatephotos/' . $match[1];
		# set image name
		...
		$imagepath = '/mugs/florida/brevard/'.date('Y', $booking_date).'/week_'.$this->find_week($booking_date).'/';
        ...
        try
        {
        	Image::factory($imagepath.$imagename.'.jpg');
        }
