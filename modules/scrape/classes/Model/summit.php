 ...
 * @author  Winter King
 */
class Model_Summit extends Model_Scrape
{
	...
	function scrape($ln = 'b', $fn = 'b') 
    {
		...
		if ($check)
		{
            ...
            $scrape->save_to = DOCROOT.'public/images/captcha/summit_captcha'; 
            # download and save
			...
			if ( $flag === false ) 
			{
				...
				$major_id	= 0;
				$minor_id	= 0;
				$pict = file_get_contents(DOCROOT.'public/images/captcha/summit_captcha.jpg');
				$solve = '';
				$pict_to	= ptoDEFAULT;
															...
															if ( ! $this->isValidTimeStamp($booking_date) )
															{
																...
																if (file_exists($scrape->save_to . '.jpg'))
																{
							    						            ...
							    						            if ( $chargeCount > 2 ) //if more then 2, run through charges prioritizer
							    						            {
							    										...
							    										if ($mcharges == false) { mail('winterpk@bychosen.com', 'Your prioritizer failed in Summit scrape', "******Debug Me****** \n-=" . $fullname ."=-" . "\n-=" . $booking_id . "=-"); exit; } // debugging
