...
class Model_Lexington extends Model_Scrape
{
	...
	*/
    ...
    {
		$home  = $this->curl_home();
		$check = preg_match('/id\=\"\_\_VIEWSTATE\"\svalue\=\"([^"]*)"/', $home,  $vs);
		if ($check)
		{
			$check = preg_match('/id\=\"\_\_EVENTVALIDATION\"\svalue\=\"([^"]*)"/', $home,  $ev);
			if ($check)
			{
				$vs    = $vs[1];
				$ev    = $ev[1]; 
			...
			} else { $this->report->info = 'ERROR: No eventvalidation found'; $this->report->update(); return false; }		
		} else { $this->report->info = 'ERROR: No viewstate found'; $this->report->update(); return false; }
	}
	...
	*/
	...
	{
		$county = 'lexington';
		$booking_id = 'lexington_' . $booking_id;
		# database validation 
			...
			if ($check)
			{
				...
				if ($check)
				{
					...
					if ($check)
					{
						...
						$charges = array();
						/*
						$details = '<td>Offense</td>
							</tr><tr class="False">
								<td>PAROLE VIOLATION THIS CHARGE IS WAY TOO LONG BECAUSE IT HAS TOO MANY CHARACTERS</td>
							</tr>
							<tr class="False">
								<td>PAROLE VIOLATION2</td>
							</tr>
						</table>';
						 * 
						 */
						$check = preg_match_all('/\<td\>Offense.*<\/table\>/Uis', $details, $match);
						if ($check)
						{
							$charges_table = preg_replace('/\<td\>Offense.*\<\/td\>/Uis', '', $match[0]);
							$check = preg_match_all('/\<td\>(.*)\<\/td\>/Uis', $charges_table[0], $matches);
							if ($check)
							{
							...
							} else { return 101; } // charges messed up
						} else { return 101; } // charges not found
