 ...
 */
class Model_Dallas extends Model 
{
	private $county 	= 'dallas';
	private $state		= 'texas';
	private $user_agent = "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.2.15) Gecko/20110303 Firefox/3.6.15";
    private $cookies 	= '/tmp/dallas_cookies.txt';
	private $csv		= '/mugs/texas/dallas/lists/dallas.csv';
	...
	*/
    function scrape($fn = '_', $ln = '_', $race = 'B', $sex = 'M') 
    {
    	$scrape = new Model_Scrape;
    	# set report variables
    	$info['Scrape'] = 'Dallas County';
    	$info['Total'] = 0;
		$info['Successful'] = 0;
		...
		if(!empty($value['Defendant']))
		{
			foreach ($index_table as $key => $value)
			{
				...
				$link = 'http://www.dallascounty.org/jaillookup/' . $link;
				$detail_links[] = $link;
			}
		...
		exit;
		//http://www.dallascounty.org/jaillookup/defendant_detail.do?recno=C600EC95-019B-F317-5B48-1474F04A9413&bookinNumber=11020957&bookinDate=1301341320000&dob=1972-12-12&lastName=ADAMS&firstName=ANTHONY&sex=Male&race=Black
	...
	* 
	*/
	function curl_index($fn, $ln, $race, $sex, $proxy = null)
	{
		$url = 'http://www.dallascounty.org/jaillookup/search.do';
	...
	*http://www.dallascounty.org/jaillookup/defendant_detail.do?recno=C600EC95-019B-F317-5B48-1474F04A9413&bookinNumber=11020957&bookinDate=1301341320000&dob=1972-12-12&lastName=ADAMS&firstName=ANTHONY&sex=Male&race=Black
	*http://www.dallascounty.org/jaillookup/defendant_detail.do?recno=C600EC95-019B-F317-5B48-1474F04A9413&bookinNumber=11020957&bookinDate=1301341320000&dob=1972-12-12&lastName=ADAMS&firstName=ANTHONY&sex=Male&race=Black 
	*http://www.dallascounty.org/jaillookup/defendant_detail.do?recno=7401DC97-2B00-8113-EFA9-D4C7BA49BE08&amp;bookinNumber=11018797&amp;bookinDate=1300669440000&amp;dob=1986-05-22&amp;lastName=ANDERSON&amp;firstName=AARON&amp;sex=Male&amp;race=Black
	 * */
					...
					{	
						...
						if (empty($ncharges)) // skip the offender if ANY new charges were found
						{
							...
							if ($check)
							{
								...
								# set image path
						        $imagepath = '/mugs/kentucky/dallas/'.date('Y', $booking_date).'/week_'.$scrape->find_week($booking_date).'/';
						        # create mugpath
								...
								if (file_exists($scrape->save_to . '.jpg')) //validate the image was downloaded
								{
							        ...
							        if ( $chargeCount > 2 ) //if more then 2, run through charges prioritizer
							        {
										...
										if ($mcharges == false) { mail('winterpk@bychosen.com', 'Your prioritizer failed in dallas scrape', "******Debug Me****** \n-=" . $fullname ."=-" . "\n-=" . $booking_id . "=-"); exit; } // debugging
