 ...
 * @url https://ijis.tarrantcounty.com/inmatesearch/Default.aspx
 */
class Model_Tarrant extends Model_Scrape
{
    private $scrape     = 'tarrant';
    private $state      = 'texas';
    private $cookies    = '/tmp/tarrant_cookies.txt';
    private $page_matches;
    ...
    */
    function scrape() 
    {
    	...
    	$flag = false;
    	$county = 'tarrant';
    	$home_url = 'https://ijis.tarrantcounty.com/inmatesearch/Default.aspx';
        $home = $this->curl_to_url($home_url);
		//$check = preg_match_all("/\<a\shref\=\"javascript\:\_\_doPostBack\(\'(.*)\'.*>(.*)\<\/a>/Uis", $searchresults, $page_matches);
		//if($check)
		//{
			//$this->page_matches = $page_matches;  
			//$count = 0;
			//for ($i = 0; $i < count($page_matches[1]); $i++)
			//{
				//$match = $page_matches[1][$i];
				
				//$post_vars['__EVENTTARGET'] = $match;
				...
				foreach ($az as $letter)
				{
					...
					$post_vars = array();
					$check = preg_match("/id\=\"\_\_VIEWSTATE\"\svalue\=\"(.*)\"/Uis", $home, $match);
					if($check)
					{
						$post_vars['__VIEWSTATE'] = $match[1];
						$check = preg_match("/id\=\"\_\_EVENTVALIDATION\"\svalue\=\"(.*)\"/Uis", $home, $match);
						if($check)
						{
							$post_vars['__EVENTVALIDATION'] = $match[1];
							$post_vars['__EVENTARGUMENT'] = '';
							$post_vars['__LASTFOCUS'] = '';
							$post_vars['ddlNumRcds'] = 20;
							$post_vars['ddlRace'] = 'All';
							$post_vars['ddlSex'] = 'Both';
							$post_vars['txtCID'] = '';
							$post_vars['txtFName'] = '';
							$post_vars['txtLName'] = $letter;
							$searchresults = $this->curl_to_url2($home_url,$post_vars);
							$page_matches = array(1);
							$check = preg_match_all("/\<a\shref\=\"javascript\:\_\_doPostBack\(\'(.*)\'.*>(.*)\<\/a>/Uis", $searchresults, $matches);
							if($check)
							{
								$page_matches = $matches[1];
							}
							foreach ($page_matches as $key => $page)
							{
								if ($key > 0)
								{
									$post_vars = array();
									$post_vars['__EVENTTARGET'] = $page;
									$check = preg_match("/id\=\"\_\_VIEWSTATE\"\svalue\=\"(.*)\"/Uis", $searchresults, $match);
									if($check)
									{
										$post_vars['__VIEWSTATE'] = $match[1];
										$check = preg_match("/id\=\"\_\_EVENTVALIDATION\"\svalue\=\"(.*)\"/Uis", $searchresults, $match);
										if($check)
										{
											$post_vars['__EVENTVALIDATION'] = $match[1];
											$post_vars['__EVENTARGUMENT'] = '';
								...
								/*
								 * Detail pages
								 * loop through each js postback and query the details page
								 * */
								$check = preg_match_all("/onclick\=\"javascript\:\_\_doPostBack\(\'(.*)\'/Uis", $searchresults, $matches);
								if($check)
								{
									foreach($matches[1] as $key => $match)
									{
										...
										if($check)
										{
											...
											{
												$post_vars['__EVENTVALIDATION'] = $match[1];
												$post_vars['__EVENTARGUMENT'] = 'Click';
												$post_vars['__LASTFOCUS'] = '';
												$post_vars['ddlNumRcds'] = 20;
												$post_vars['ddlRace'] = 'All';
												$post_vars['ddlSex'] = 'Both';
												...
												$post_vars['txtFName'] = '';
												$post_vars['txtLName'] = $letter;
												$details = $this->curl_to_url2($home_url,$post_vars);
    ...
    */
    ...
    {
    	$check = preg_match('/\<input\sname\=\"txtCID\"\stype\=\"text\"\svalue\=\"(.*)\"/Uis', $details_page, $booking_id);
		$booking_id = $county .'_'. $this->clean_string_utf8($booking_id[1]);
		if(isset($booking_id))
		{
			$check = preg_match('/txtFName\"\stype\=\"text\"\svalue\=\"(.*)\"/Uis', $details_page, $fullname);
			$check = preg_match('/txtLName\"\stype\=\"text\"\svalue=\"(.*)\"/Uis', $details_page, $lastname);
			
			$offender = Mango::factory('offender', array(
            	'booking_id' => $booking_id
        	))->load(); 
			if(!$offender->loaded()) // If offender is not loaded then skip
			{
				$check = preg_match('/txtFName\"\stype\=\"text\"\svalue\=\"(.*)\"/Uis', $details_page, $fullname); 
				if($check)
				{
					...
					if($check)
					{
						$lastname = strtoupper(trim($lastname[1]));
						$check = preg_match_all('/bgcolor\=\"\#EEEEEE.*title\=.*\>\<.*\>(.*)\</Uis', $details_page, $charges);
						if($check)
						{
							$charges = $this->clean_string_utf8($charges[1][0]);
							$check = preg_match('/title\=\"Arrest\sDate\:(.*)\"/Uis', $details_page, $booking_date);
							if($check)
							{
								$booking_date = strtotime(trim($this->clean_string_utf8($booking_date[1])));
								$check = preg_match('/imgMugshot\"\ssrc\=\"(.*)\"/Uis', $details_page, $image_id);
								if($check)
								{
									$image_id = $image_id[1];
									$image_link = "https://ijis.tarrantcounty.com/inmatesearch/" . $image_id;
									$check = preg_match('/showmugshot\.aspx\?url\=\\\\\\\\NAS2IJIS01\\\\MUGSHOTS\\\\.*\\\\.*\\\\(.*)$/Uis', $image_id, $basicimage);
									$basicimage = $basicimage[1];
									$imagename = date('(m-d-Y)', $booking_date) . '_' . $lastname . '_' . $firstname . '_' . $booking_id;
									$imagepath = '/mugs/texas/'.$county.'/'.date('Y', $booking_date).'/week_'.$this->find_week($booking_date).'/';
									$county_directory = '/mugs/texas';
									$this->make_county_directory($county_directory);
									$extra_fields = array();
									$check = preg_match('/txtRace\"\stype\=\"text\"\svalue\=\"(.*)\"/Uis', $details_page, $match);
									if($check)
									{
										$extra_fields['race'] = $this->race_mapper($this->clean_string_utf8($match[1]));
									}
									$check = preg_match('/txtSex\"\stype\=\"text\"\svalue\=\"(.*)\"/Uis', $details_page, $match);
									if($check)
									{
