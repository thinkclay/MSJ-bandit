 ...
 */
class Model_Stark extends Model_Scrape
{
	...
	public function __construct()
  {
    ...
    function  scrape($from, $to)
    {
		$index = $this->curl_index($from, $to);
		//<a href=/cgi/progcgi.exe?program=master?fid=seorj?recid=1272748>35902</a>
							...
							if (!empty($booking_id))
							{
								if (!empty($booking_date))
								{
									if (!empty($charges))
									{
										...
										$check = preg_match('/Eyes\:[^>]*\>([^<]*)\</', $details, $eye_color);
										...
										$check = preg_match('/(\/photo\/seorj\/[0-9]*\.jpg)/', $details, $img);
										if ( ! $check)
										{
											return 102;
										}
