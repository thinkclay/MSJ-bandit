 ...
 */
class Model_Licking extends Model_Scrape
{
    private $scrape     = 'licking'; //name of scrape goes here
    private $county     = 'licking'; // if it is a single county, put it here, otherwise remove this property
    private $state      = 'ohio'; // state goes here
    private $cookies    = '/tmp/licking_cookies.txt'; // replace with <scrape name>_cookies.txt
    ...
    */
    function scrape() 
    {
        $this->raw_dir = '/raw/ohio/licking/upload/';
        $this->extract_dir = '/raw/tmp/licking';
		$file = null;
		...
		if ($handle = opendir($this->extract_dir . '/')) 
		{
	        $count = 0;
	        while (false !== ($entry = readdir($handle))) 
	        {
		...
		// loop through each offender and group together the ones with matching NAME_ID fields
		$offenders = array();
		foreach ($xml->IndividualsThatWereArrested as $offender)
		{
			$offenders[(int)$offender->NAME_ID]['first_name'] = (string)$offender->FIRST_NAME;
			$offenders[(int)$offender->NAME_ID]['last_name'] = (string)$offender->LAST_NAME;
			$offenders[(int)$offender->NAME_ID]['booking_date'] = (string)$offender->BOOKDATE;
			$offenders[(int)$offender->NAME_ID]['charges'][] = (string)$offender->CHARGE;
		}
		$count = 0;
		foreach ($offenders as $booking_id => $offender)
		{
			$extraction = $this->extraction($offender, $booking_id);
    ...
    * 
    */
    function extraction($details, $bid)
    {
        $booking_id = $this->county . '_' . $bid; // set the booking_id to <scrapename>_<booking_id>
        ...
        if ( ! $offender->loaded() ) 
        {
        	$firstname = strtoupper(trim($details['first_name']));
            $lastname = strtoupper(trim($details['last_name']));
			$booking_date = strtotime(trim($details['booking_date']));
			if (!empty($firstname) AND !empty($lastname) AND !empty($booking_date))
			{
				// set the charges variable
                $charges = array();
				foreach ($details['charges'] as $charge)
				{
					$charges[] = $this->charge_trim($this->clean_string_utf8(htmlspecialchars_decode(str_replace('&nbsp;', ' ', trim($charge)), ENT_QUOTES)));
				}
                ...
                if (empty($ncharges)) // skip the offender if ANY new charges were found
                {
					...
					if ($handle = opendir($this->extract_dir . '/')) 
					{
	        			...
	        			{
	        				//echo $bid;
							if (strpos($entry, (string)$bid) !== false)
							{
								$flag = true;
								$image_file = $entry;
								break;
                    ...
                    $imagepath = '/mugs/ohio/licking/'.date('Y', $booking_date).'/week_'.$this->find_week($booking_date).'/';
                    $mugpath = $this->set_mugpath($imagepath);
					if (file_exists($this->extract_dir . '/' . $bid.'.jpg'))
                    {
						// move it to the image path
                        copy($this->extract_dir . '/' . $bid.'.jpg', $imagepath . $imagename);
    ...
    {
    	// clear out the extration directory
