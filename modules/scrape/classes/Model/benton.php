 ...
 * @package Scrape
 * @author Bryan Galli
 * @url http://www.co.benton.or.us/sheriff/corrections/jms_reports.php?d=i
 */
class Model_Benton extends Model_Scrape
{
    private $scrape     = 'benton';
    private $state      = 'oregon';
    private $cookies    = '/tmp/benton_cookies.txt';
    ...
    */
    ...
    {
    	$county = 'benton';
    	$home_url = 'http://www.co.benton.or.us/sheriff/corrections/jms_reports.php?d=i';
        $home = $this->curl_to_url($home_url);
        $check = preg_match_all('/\<a\shref\=\"jms\_reports\_view\.php\?d\=i\&f\=(.*)\"/Uis', $home, $matches);
		if($check)
		{
    ...
    */
    ...
    {
    	$theurl = "http://www.co.benton.or.us/sheriff/corrections/jms_reports_view.php?d=i&f=" . $offender_link;
    	$details_page = $this->curl_to_url($theurl);
    	$check = preg_match('/Booking.*\<td.*\>(.*)\</Uis', $details_page, $booking_id);
		...
		if(isset($booking_id))
		{
			...
			if(!$offender->loaded())
			{
				$check = preg_match('/Inmate\sName.*\<td.*\>(.*)\</Uis', $details_page, $name);
				if($check)
				{
					...
					$check = preg_match_all('/2\"\>Offense.*\<td.*\>(.*)\</Uis', $details_page, $charges);
					if($check)
					{
						$charges = $this->clean_string_utf8($charges[1]);
						$check = preg_match('/Booking\sDate\:\<\/td\>.*\<td.*>(.*)\</Uis', $details_page, $booking_date);
						if($check)
						{
							$booking_date = strtotime(trim($this->clean_string_utf8($booking_date[1])));
							$check = preg_match('/src\=\"\/sheriff\/corrections\/documents\/inmates\/(.*)\"/Uis', $details_page, $image_id);
							if($check)
							{
								$image_id = $this->clean_string_utf8($image_id[1]);
								if($image_id == 'NOPIC.JPG')
									return 101;
								$image_link = 'http://www.co.benton.or.us/sheriff/corrections/documents/inmates/' . $image_id;
