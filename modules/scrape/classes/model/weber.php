 ...
 * @package Scrape
 * @author 	Winter King
 * @url 	http://www.co.weber.ut.us/offender/offendersearch.php
 */
class Model_Weber extends Model_Scrape
{
    private $scrape     = 'weber'; //name of scrape goes here
	private $county 	= 'weber'; // if it is a single county, put it here, otherwise remove this property
    private $state      = 'weber'; // state goes here
    private $cookies    = '/tmp/weber_cookies.txt'; // replace with <scrape name>_cookies.txt
    ...
    */
    ...
    {
    	$count = 0;
    	foreach ($this->alphabet as $letter1)
		{
			foreach ($this->alphabet as $letter2)
			{
				$index = $this->curl_index($letter1, $letter2);
				//http://www.co.weber.ut.us/offender/offenderresults.php?ID=304162"
				$check = preg_match_all('/offenderresults\.php\?ID\=(.*)\"/Uis', $index, $matches); // get an array of booking_ids
				if ($check)
				{
    ...
    * 
    */
    function curl_index($ln, $fn)
    {    	
    	$post = 'ltname='.$ln.'&ftnam='.$fn.'&submit=Submit'; // build out the post string here if needed
        $url = 'http://www.co.weber.ut.us/offender/offendersearch.php';  // this will be the url to the index page
        $ch = curl_init();
    ...
    */
    ...
    {
        $url = 'http://www.co.weber.ut.us/offender/offenderresults.php?ID='.$booking_id;
        $ch = curl_init();   
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_COOKIEFILE, $this->cookies);
        curl_setopt($ch, CURLOPT_COOKIEJAR, $this->cookies);
    ...
    */
    ...
    {
    	//$booking_date = strtotime('-10 years');
		...
		$check = preg_match('/NAME\:\:\<\/th\>.*\<td.*\>(.*)\</Uis', $details, $match);
		if ( ! $check)
		{
			return 101;
		}
		$name = $this->clean_string_utf8(htmlspecialchars_decode(trim($match[1]), ENT_QUOTES));
		$name = preg_replace("/\s\s+/", " ", $match[1]);
		...
		//$booking_date = strtotime('-'.rand(0, 12).' months -10 years');
		$booking_date = time();
		...
		*/
		...
		// "http://www.co.weber.ut.us/offender/pic.php?PICID=175459 "
		$check = preg_match('/pic\.php\?PICID\=(.*)\s\"/Uis', $details, $match);
		if ( ! $check)
		{
			return 102; // Image link not found
		}
	    $image_link = 'http://www.co.weber.ut.us/offender/pic.php?PICID=' . $match[1];
		# set image name
        ...
        try
        {
			...
			$check = $this->mugStamp($imgpath, $firstname . ' ' . $lastname, ' ');
