 ...
 * @url 	http://www.sjso.org
 */
class Model_Stjohns extends Model_Scrape
{
    private $scrape     = 'stjohns'; //name of scrape goes here
	private $county 	= 'stjohns'; // if it is a single county, put it here, otherwise remove this property
    private $state      = 'stjohns'; // state goes here
    private $cookies    = '/tmp/stjohns_cookies.txt'; // replace with <scrape name>_cookies.txt
    ...
    */
    ...
    function scrape() 
    {
		...
		$check = preg_match_all('/<thead\>[\s\t\r\n]+<tr\>[\s\t\r\n]+<td\sclass\=\"SearchHeader\"\scolspan\=\"2\">.*\<\/table>.*\<table.*\>.*\<\/table\>/Uis', $index, $matches);
		if ( ! $check)
			die('Details not found.');
		$details_pages = $matches[0];
		foreach ($details_pages as $details)
		{
    ...
    */
    function curl_index()
    {
        $url = 'http://www2.sjso.org/smartweb/jail.aspx';
        $ch = curl_init();   
    ...
    */
    ...
    {
    	$check = preg_match('/Inmate\sNumber\:\<\/B\>(.*)\<\//Uis', $details, $match);
		if ( ! $check)
		{
			return 101;
		}
        $booking_id = $this->scrape . '_' . trim($match[1]); // set the booking_id to <scrapename>_<booking_id>
		...
		$check = preg_match('/SearchHeader.*\>(.*)\&nbsp\;/Uis', $details, $match);
		if ( ! $check)
		{
			return 101;
		}
		...
		$check = preg_match('/Booking\sDate\:.*\<td\>(.*)\</Uis', $details, $match);
		if ( ! $check)
		{
			return 101; 	
		}
		...
		$check = preg_match('/\<table\sclass\=\"JailViewCharges\".*\>.*\<\/table\>/Uis', $details, $match);
		if ( ! $check)
		{
			return 101;
		}
		...
		$charges_rows = $matches[0];
		unset($charges_rows[0]);
		$charges = array();
		foreach ($charges_rows as $row)
		{
			$check = preg_match('/\<td.*>.*\<\/td>.*<td.*>.*<\/td>.*<td.*>(.*)\<\/td\>/Uis', $row, $match);
			if ( ! $check)
				return 101;
	    ...
	    $image_link = 'http://www2.sjso.org/smartweb/ViewImageFull.aspx?ecsoid=' .str_replace('stjohns_', '', $booking_id);
		# set image name
		...
		$imagepath = '/mugs/florida/stjohns/'.date('Y', $booking_date).'/week_'.$this->find_week($booking_date).'/';
