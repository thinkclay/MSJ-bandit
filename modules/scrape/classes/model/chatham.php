...
/**
 * Model_Chatham
 * @TODO FIX THIS Undefined offset: 1 ~ MODPATH/scrape/classes/model/chatham.php [ 104 ]
 * 
 * 
 * @package Scrape
 * @author Bryan Galli
 * @url http://chathamsheriff.org/Corrections/Operations/Bookings72hrs.aspx
 */
class Model_Chatham extends Model_Scrape
{
    private $scrape     = 'chatham';
    private $state      = 'georgia';
    private $cookies    = '/tmp/chatham_cookies.txt';
    ...
    */
    ...
    {
    	$county = 'chatham';
    	$home_url = 'http://chathamsheriff.org/Corrections/Operations/Bookings72hrs.aspx';
        $home = $this->curl_to_url($home_url);
        $check = preg_match_all('/style\=\"background\-color\:\#FFF\;\sborder\:1px\ssolid\s\#ddd\;\"\>(.*)\<\/table\>/Uis', $home, $matches);
		if($check)
		{
    ...
    */
    ...
    {
    	$check = preg_match('/\<strong\>Arrest\s\#\:\<\/strong\>(.*)\<\/td\>/Uis', $details_page, $booking_id);
		...
		if(isset($booking_id))
		{
			...
			if(!$offender->loaded())
			{
				$check = preg_match('/title\=\"(.*)\"/Uis', $details_page, $name); 
				if($check)
				{
					...
					$check = preg_match_all('/br\s\/\>.*\s\-\s(.*)\s\-.*\</Uis', $details_page, $charges);
					if($check)
					{
						$charges = $this->clean_string_utf8($charges[1]);
						$check = preg_match('/arrest\sdate\:\<\/strong\>(.*)\s/Uis', $details_page, $booking_date);
						if($check)
						{
							$booking_date = strtotime(trim($this->clean_string_utf8($booking_date[1])));
							$check = preg_match('/img\ssrc\=.*\=(.*)\"/Uis', $details_page, $image_id);
							if($check)
							{
								$image_id = $this->clean_string_utf8($image_id[1]);
								$image_link = 'http://dinphotos.chathamcounty.org/default.aspx?din=' . $image_id;
								$imagename = date('(m-d-Y)', $booking_date) . '_' . $lastname . '_' . $firstname . '_' . $booking_id;
								$imagepath = '/mugs/georgia/'.$county.'/'.date('Y', $booking_date).'/week_'.$this->find_week($booking_date).'/';
								$county_directory = '/mugs/georgia';
								$this->make_county_directory($county_directory);
								$extra_fields = array();
								$check = preg_match('/race\:\<\/strong\>(.*)\</Uis', $details_page, $match);
								if($check)
								{
									$extra_fields['race'] = $this->race_mapper($this->clean_string_utf8($match[1]));
								}
								$check = preg_match('/sex.*\<\/strong\>(.*)\</Uis', $details_page, $match);
								if($check)
								{
