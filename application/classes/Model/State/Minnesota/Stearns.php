 ...
 */
class Model_State_Minnesota_Stearns extends Model_Scrape
{
    private $scrape     = 'stearns'; //name of scrape goes here
	private $county 	= 'stearns'; // if it is a single county, put it here, otherwise remove this property
    private $state      = 'Minnesota'; // state goes here
    private $cookies    = '/tmp/stearns_cookies.txt'; // replace with <scrape name>_cookies.txt
    ...
    */
    function scrape() 
    {
		...
		$check = preg_match_all('/(jail\_inmate\_link.*)\"/Uis', $index, $matches); // get an array of booking_ids
		if ($check)
		{
			//In this case 
    ...
    */
    function curl_index()
    {
        $url = 'http://www.clickcomplete.com/jail/default.cfm?Order=5&PID=1.3&inq_key=10023&action=Park_List&type=Desc&searchDate=&searchEndDate=&Crit1_Operator=&Crit1_Value=&Crit2_Operator=&Crit2_Value=';  // this will be the url to the index page
        $ch = curl_init();   
    ...
    */
    ...
    {
        $url = 'http://www.clickcomplete.com/jail/default.cfm?PID=1.3&inq_key=10023&action=editprofile&'.$booking_id;
        $ch = curl_init();   
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_COOKIEFILE, $this->cookies);
        curl_setopt($ch, CURLOPT_COOKIEJAR, $this->cookies);
    ...
    */
    ...
    {
        $booking_number = explode('&', $booking_id);
        $booking_number = str_replace('jail_inmate_link=', "", $booking_number[0]);
        $booking_id = $this->scrape . '_' . $booking_number; // set the booking_id to <scrapename>_<booking_id>
		...
		//Get Last Name
		$check = preg_match('/class\=\"desc.*Last\sName.*formFieldsmall.*\>(.*)\</Uis', $details, $match);
		if(!$check)
		{
			return 101;
		} 
		$lastname = $this->clean_string_utf8(htmlspecialchars_decode(str_replace('&nbsp;', ' ', trim($match[1])), ENT_QUOTES));
	
		//Get First Name
		$check = preg_match('/class\=\"desc.*First\sName.*formFieldsmall.*\>(.*)\</Uis', $details, $match);
		if(!$check)
		{
			return 101;
		} 
		$firstname = $this->clean_string_utf8(htmlspecialchars_decode(str_replace('&nbsp;', ' ', trim($match[1])), ENT_QUOTES));
	
		//Get Booking Date
		$check = preg_match('/class\=\"desc.*Booking\sDate.*formFieldsmall.*\>(.*)\</Uis', $details, $match);
		if(!$check)
		{
			return 101;
		}  
		...
		$charges = array();
		$check = preg_match('/class\=\"desc.*\>Charge\<.*formFieldsmall.*\>(.*)\</Uis', $details, $match);
		$total_charges = explode('/',$match[1]);
		foreach ($total_charges as $charge)
		{
		...
		$check = preg_match('/class\=\"desc.*\>Age\<.*formFieldsmall.*\>(.*)\syears.*\</Uis', $details, $match);
		if ($check)
		{
			$extra_fields['age'] = $match[1];
		}
	
		$check = preg_match('/class\=\"desc.*\>Gender\<.*formFieldsmall.*\>(.*)\</Uis', $details, $match);
		if ($check)
		{
			$extra_fields['gender'] = strtoupper(trim($match[1]));
		}		
		
		$check = preg_match('/class\=\"desc.*\>Race\<.*formFieldsmall.*\>(.*)\</Uis', $details, $match);
		if ($check)
		{
			// this will map race names to our standard format for races
			// ie. African American becomes Black, 
			$extra_fields['race'] = $this->race_mapper(strtoupper($match[1]));
		} 
		
		// now get the image link and download it
		$check = preg_match('/\<img\ssrc\=\"(https\:\/\/www\.clickcomplete\.com\/jail\/jmsimages.*)\"/Uis', $details, $match);
		if (strpos($match[0], 'no-photo') !== false) {
			echo 'Error: No Photo Available  - ' . $lastname . ' ' . $firstname . '<br />';
			return 102;
		}
		if ( ! $check)
		{
		...
		$imagepath = '/mugs/minnesota/stearns/'.date('Y', $booking_date).'/week_'.$this->find_week($booking_date).'/';
