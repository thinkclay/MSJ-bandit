 ...
 */
class Model_State_Florida_Indianriver extends Model_Scrape
{
    ...
    */
    function scrape($booking_date = null) 
    {
    	if ( ! $booking_date)
    	{
    		$booking_date = date('m/d/Y', (time()-86400));	// sets yesterday's date and checks for yesterday's bookings
    	}
    ...
    */
    function extraction($details, $booking_id)
    {
        ...
        if ( $offender->loaded() ) 
        {
        	...
        	echo 'offender already exists. skip this offender';
		} 
    	...
    	*/
		...
		{
			return 101;
			echo 'preg_match of check failed line 171';
		}
		...
		{
			return 101;
			echo 'preg_match of check failed line 190';
		}
		// Format for weird characters
		...
		*/
		...
		$check = preg_match('/nw-docs\/(.*)\.mug/Uis', $details, $match);
		if ( ! $check)
		{
			echo '102';
	    ...
	    $image_link = 'http://www.ircsheriff.org/nw-docs/' . $match[1] . '.mug';
		# set image name
        ...
        if ( $chargeCount > 2 ) //if more then 2, run through charges prioritizer
        {
			...
			if ($mcharges == false) { mail('bustedreport@gmail.com', 'Your prioritizer failed in marion scrape', "******Debug Me****** \n-=" . $fullname ."=-" . "\n-=" . $booking_id . "=-"); exit; } // debugging
